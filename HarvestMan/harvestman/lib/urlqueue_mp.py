# -- coding: utf-8
""" Urlqueue - multiprocessing version
(not working yet)
"""
...
from harvestman.lib import crawler_mp
...
class HarvestManCrawlerState(Singleton):
    ...
    def state_callback(self, thread, state):
        ...
        elif state == crawler_mp.FETCHER_PUSHED_URL:
            # Push count for fetcher threads
            self.fpush += 1
        elif state == crawler_mp.CRAWLER_PUSHED_URL:            
            # Push count for fetcher threads
            self.cpush += 1
        elif state == crawler_mp.FETCHER_GOT_DATA:
            # Get count for fetcher threads
            self.fgets += 1
        elif state == crawler_mp.CRAWLER_GOT_DATA:
            # Get count for fetcher threads
            self.cgets += 1            
        elif state == crawler_mp.THREAD_SLEEPING:
        ...
        elif state in (crawler_mp.FETCHER_WAITING, crawler_mp.CRAWLER_WAITING):
            if self.end_state():
...
class HarvestManCrawlerQueue(object):
    ...
    def configure(self):
        ...
        try:
            self.basetracker = crawler_mp.HarvestManUrlFetcher( 0, self.baseurl, True )
        except Exception, e:
            print "Fatal Error:",e
            hexit(1)
        ...
        #    self.basetracker = crawler_mp.HarvestManUrlDownloader( 0, self.baseurl, False )
    ...
    def crawl(self):
        ...
        self.stateobj.numfetchers = 1 #int(0.75*self.configobj.maxtrackers)
        self.stateobj.numcrawlers = 1 #self.configobj.maxtrackers - self.stateobj.numfetchers

        # self.basetracker.setDaemon(True)
        self.basetracker.start()

        evt = SleepEvent(0.1)
        while self.stateobj.get(self.basetracker) == crawler_mp.FETCHER_WAITING:
            evt.sleep()
        ...
        for x in range(1, self.stateobj.numfetchers):
            t = crawler_mp.HarvestManUrlFetcher(x, None)
            self.add_tracker(t)
            t.start()

        for x in range(self.stateobj.numcrawlers):
            t = crawler_mp.HarvestManUrlCrawler(x, None)
            self.add_tracker(t)
            t.start()
    ...
    def dead_thread_callback(self, t):
        ...
        try:
            ...
            if role == 'fetcher':
                new_t = crawler_mp.HarvestManUrlFetcher(t.get_index(), None)
            elif role == 'crawler':
                new_t = crawler_mp.HarvestManUrlCrawler(t.get_index(), None)
    ...
    def push(self, obj, role):
        ...
        if role == 'crawler' or role=='tracker' or role =='downloader':
            ...
            # self.stateobj.set(ct, crawler_mp.CRAWLER_PUSH_URL)
            ntries += 1
            ...
            # self.stateobj.set(ct, crawler_mp.FETCHER_PUSH_URL)                                
    ...
    def _kill_tracker_threads(self):
        ...
        for tracker in self.trackers:
            ...
            try:
                ...
                logconsole(str(e))
            except crawler_mp.HarvestManUrlCrawlerException, e:
                pass
